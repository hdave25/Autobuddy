...
export default class UpdateUserProfile extends Component {
    ...
    async getPrevDetails() {
        ...
        let ref1 = rootRef.child("suratclassifieds/AdminUsers/"+this.state.userKey+"/username");
        await ref1.once("value").then(function (snapshot) {
            db_Username=snapshot.val();

        });
        let ref2 = rootRef.child("suratclassifieds/AdminUsers/"+this.state.userKey+"/userId");
        await ref2.once("value").then(function (snapshot) {
            db_Id=snapshot.val();

        });

        let ref4 = rootRef.child("suratclassifieds/AdminUsers/"+this.state.userKey+"/SuperUser");
        await ref4.once("value").then(function (snapshot) {
            db_superUser=snapshot.val();

        });

        this.setState({name: db_Username});
        this.setState({userId: db_Id});
        this.setState({superUser: db_superUser});
    }
    ...
    async updateData() {
        ...
        let config = {
            ...
            firebase.initializeApp(config);
            let rootRef = firebase.database().ref();
            let ref = rootRef.child("suratclassifieds/AdminUsers/"+this.state.userKey);
            await ref.set({
